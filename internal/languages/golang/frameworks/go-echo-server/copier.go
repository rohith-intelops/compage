...
func (c *Copier) copyEchoClientResourceFiles(restClient *corenode.RestClient) error {
	/// add resource-specific data to map in c needed for templates.
...
func (c *Copier) CreateRestClients() error {
	...
	if c.HasRestClients {
		...
		for _, client := range c.RestClients {
			if err := c.copyEchoClientResourceFiles(client); err != nil {
